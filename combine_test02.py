...
def create_key():
    ...
    pri_key = open("pri_key.pem",'wb')
    pri_key.write(private_key)
    pri_key.close()
...
def decryptor():
    ...
    pri_key = open("pri_key.pem",'rb')
    private_key = pri_key.read()
    pri_key.close()

    encrypted = raw_input("要解密的: ")
    encrypted_txt = open("encrypted.txt",'rb')
    encrypted = encrypted_txt.read()
    encrypted_txt.close()
...
def encryptor():
    ...
    encrypted_txt = open("encrypted.txt",'wb')
    encrypted_txt.write(encrypted)
    encrypted_txt.close()
    print encrypted


def main():
    option = raw_input("生成密钥(c) or 加密(e) or 解密(d): ")
    if option == 'e':
        encryptor()
    elif option == 'd':
        decryptor()
